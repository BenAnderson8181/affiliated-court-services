...
const Depression: NextPage = () => {
    ...
    const questionQuery = api.question.list.useQuery({ assessmentURL: 'depression' });
    ...
    const domainScoreCardMutation = api.domainScoreCard.depression.useMutation();
    ...
    const depressionAssessment = assessmentsQuery.data?.find((assessment) => assessment.url === 'depression');
    const depressionRequiredAssessments = requiredAssessmentsQuery.data?.filter((assessment) => assessment.assessment.url === 'depression');
    const incompleteDepressionRequiredAssessments = depressionRequiredAssessments?.filter((assessment) => assessment.completed !== true);

    if (incompleteDepressionRequiredAssessments?.length === 0) {
    ...
    const onSubmit = async (data: Data) => {
        const depressionAssessmentId = depressionAssessment?.id;
        ...
        const answersSum = answers.reduce((acc, answer) => acc + parseInt(answer.answer), 0);

        if (answersSum < 8) {
            console.error('sum of answers must be at least 8')
            setShowSuccessAlert(true);
            return;
        }

        const { tscore, se, risk } = getTScore(answersSum);
    ...
    return (
        <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-[#2f0f5b] to-[#6941a2] text-slate-700 pt-5">
            ...
            <div className="flex flex-col items-center justify-center border border-indigo-700 rounded shadow-xl shadow-purple-900 bg-slate-100 w-5/6 p-5">
                <h1 className="text-xl font-bold mb-3">{depressionAssessment?.description}</h1>
                ...
                &#34;no interest or pleasure in doing things&#34; and/or &#34;feeling down, depressed, or hopeless&#34; at a mild or greater level of severity.  The questions below ask about these
