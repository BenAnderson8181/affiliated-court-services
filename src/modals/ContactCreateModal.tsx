...
const ContactCreateModal = ({ accountId, onClose, onRefresh }: Props) => {
    ...
    const contactCreateMutation = api.contact.create.useMutation();
    ...
    if (agentTypeQuery.isLoading) {
        return <Loading type="Modal" />
    }

    if (agentTypeQuery.isError) {
    ...
    const onSubmit = async (contact: ContactSchemaType) => {
        ...
        const result = await contactCreateMutation.mutateAsync({
            ...contact,
            clientId: accountId,
            isInSystem,
            agentId: _agentId
    ...
    const handleSearchResultClick = ({ firstName, lastName, phone, email, id, agentTypeId, agency}: Account) => {
    ...
    return (
        <div className="h-full w-full p-0 flex flex-col">
            <div className="flex flex-row justify-center text-3xl py-2 bg-gradient-to-b from-[#2f0f5b] to-[#6941a2] text-slate-100 w-full rounded-t-md">Add Contact</div>
            ...
            {
                showErrorAlert &&
                <div className="border border-orange-500 round mt-4 p-3 text-slate-700 text-2xl">Something went wrong creating the contact.  Please try again or contact our office.</div>
            }
        </div>
    )
}
